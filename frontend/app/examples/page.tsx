...
export default function ExamplesPage() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:to-slate-950">
      ...
      <main className="max-w-6xl mx-auto px-4 py-16">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-3">Public demo walls</h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {DEMO_PRESETS.map((preset) => (
            <Link
              ...
              className="group rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 hover:border-indigo-300 hover:shadow-lg transition-all"
            >
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:text-indigo-400">{preset.name}</h2>
                <span className="text-xs font-mono px-2 py-1 rounded bg-indigo-50 text-indigo-700 dark:text-indigo-300">{preset.theme}</span>
              </div>
